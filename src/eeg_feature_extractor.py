...
def extract_eeg_feature_for_sentences(sentence_list):
    ...
    for participant in range(1,42):   # change to number of participant
        ...
        if flag!=0:print(len(matrix[flag][0]),len(matrix))
        else:print("Vocabulary Issue")
...
def extract_eeg_window_feature_for_sentences(sentence_list):
    main_list = []
    for participant in range(1,15):   # change to number of participant
        if participant == 5:continue   # lol
        print('Participant : ',participant,' in progress')
        try:
          number = ''
          if participant<10:number=number+'0'+str(participant)
          ...
          for single_sentence in sentence_list:
              ...
              for single_word in word_list:
                  ...
                  index = int(index)
                  onset, offset = int(mat_file['proc']['trl'][0][0][index-1][0]),int(mat_file['proc']['trl'][0][0][index-1][1])
                  filtered_data = raw_file['raw']['trial'][0][0][0][0][:,onset-1:offset]  # [62,651]                  
                  concat_vec = []
                  for item in [275, 300, 325, 350, 375, 400]:  # Decoding word and category-specific spatiotemporal representations from MEG and EEG
